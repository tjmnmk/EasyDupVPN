...
class Tun():
    def __init__(self):
        self._device = config.get_device_name()
        self._mtu = config.get_mtu()
        self._ip_address = config.get_tun_address_ipv4()
        self._netmask = config.get_tun_netmask_ipv4()
        self._ipv6_address = config.get_tun_address_ipv6()
        self._ipv6_netmask = config.get_tun_netmask_ipv6()
        self._tun = self._tun_open(self._device)

        self._create_tun_interface()

    def _create_tun_interface(self):
        loguru.logger.info(f"Creating TUN interface {self._device} with MTU {self._mtu}")

        sh.ip('tuntap', 'add', 'dev', self._device, 'mode', 'tun')
        sh.ip('link', 'set', 'dev', self._device, 'up')
        sh.ip('link', 'set', 'dev', self._device, 'mtu', str(self._mtu))
        if self._ip_address and self._netmask:
            sh.ip('addr', 'add', 'dev', self._device, self._ip_address + '/' + self._netmask)
        if self._ipv6_address and self._ipv6_netmask:
            sh.ip('-6', 'addr', 'add', 'dev', self._device, self._ipv6_address + '/' + self._ipv6_netmask)
    ...
    def tun_read(self):
        loguru.logger.debug("Reading from TUN device")
        try:
